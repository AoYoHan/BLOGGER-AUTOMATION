...
def generate_blog_post(keyword: str, research: dict, tone: str = "전문적이면서 친근한", color: str = "#3d94f6", target_year: str = "2026") -> dict:
    ...
    Returns:
        dict: {"title", "meta_description", "tags", "content" (HTML), "subtopics"}
    """
...
{{
    "title": "매력적인 블로그 제목",
    "meta_description": "최신 트렌드를 반영한 메타 설명",
    "tags": ["{target_year}년", "키워드1", "키워드2", "키워드3", "키워드4"],
...
"""
    ...
    try:
        ...
        result.setdefault("meta_description", f"{keyword}에 대한 상세 가이드")
        result.setdefault("tags", [keyword])
        ...
        return {
            "title": f"{keyword} 가이드",
            "meta_description": f"{keyword}에 대한 상세 가이드입니다.",
            "tags": [keyword],
            "subtopics": [],
